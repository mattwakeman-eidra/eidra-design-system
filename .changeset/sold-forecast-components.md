...
- **Density** — `StatisticBar`, `SegmentBar`, `StatusStrip`, and `Statistic` now honour the ambient `data-density="compact"` scope (tighter padding/gaps, smaller numerals); `SegmentedControl`/`FilterSelect` scale via the control-size tokens. DataGrid compact rows are tighter (~24px).
