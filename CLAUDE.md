...
- `packages/tokens/` — design tokens. **Edit DTCG JSON in `tokens/`**, never the generated CSS/TS. `pnpm build:tokens` regenerates `dist/eidra-tokens.css` (CSS variables) + typed TS, plus the Tailwind v4 bridge: `tailwind.css` (maps tokens onto Tailwind's `@theme`) and `tailwind-reset.css` (the **opt-in** reset that drops Tailwind's defaults). Eidra Sans fonts in `fonts/`, declared in `fonts.css`.
...
- **Opinionated but overridable** (ADR-0009). Ship sensible, on-token defaults, but never style anything a consumer can't override, and never modify the host outside the DS scope. Global resets (e.g. the Tailwind default-theme reset) ship as **opt-in** imports, never bundled into a default entrypoint. If a default isn't overridable by ordinary consumer CSS, it's a bug.
...
- `pnpm lint` — ESLint (TS/TSX) + Stylelint (CSS Modules); `pnpm lint:fix` auto-fixes. ESLint is **not** type-aware (tsc owns types) so it stays fast; it fails CI on errors only (warnings are informational). Stylelint enforces "colours come from tokens" (`color-no-hex`/`color-named`). Configs: `eslint.config.js`, `stylelint.config.js`.
- `pnpm format` / `pnpm format:check` — Prettier across the repo (`.prettierrc.json`). CI's `lint` job runs `pnpm lint` + `pnpm format:check`.
- **Git hooks** (Husky, installed via the `prepare` script on `pnpm install`): `pre-commit` runs lint-staged (`eslint`/`stylelint`/`prettier --fix` on staged files); `commit-msg` runs commitlint (Conventional Commits, `commitlint.config.js`). Local convenience only — CI is the real gate; bypass with `--no-verify`.
