...
const CLUSTERS: Cluster[] = [
  {
    ...
    choices: [
      { use: 'Statistic', when: 'One headline metric, optionally with a delta, progress bar, or accent left-border.' },
      { use: 'StatisticBar', when: 'A horizontal strip of several figures separated by dividers — a compact KPI header.' },
      { use: 'StatusStrip', when: 'A "heat row" — equal-width cells each tinted by a RAG status (e.g. monthly momentum).' },
      { use: 'SegmentBar', when: 'A single bar split into proportional, tinted segments (a composition of a whole).' },
      { use: 'Meter / Progress', when: 'A scalar value within a range (Meter) vs. task completion incl. indeterminate (Progress).' },
    ],
  },
  {
    ...
    choices: [
      { use: 'ToggleGroup', when: 'A cluster of buttons — single- or multi-select. Use appearance="segmented" for a filled view-switcher (formerly SegmentedControl).' },
  ...
  },
  {
    ...
    choices: [
      ...
      { use: 'FilterSelect', when: 'A compact filter pill summarising a multi-select (dashboards, tables).' },
      { use: 'Menu / ContextMenu', when: 'A list of actions — button-triggered (Menu) vs. right-click (ContextMenu).' },
    ],
  },
  {
    ...
    choices: [
      ...
      { use: 'Accordion / Collapsible', when: 'Multi-section disclosure with headings (Accordion) vs. a single collapsible region.' },
    ],
  },
  {
    ...
    choices: [
      ...
      { use: 'Toast', when: 'Transient, stacked notification that auto-dismisses (async results).' },
      { use: 'Badge', when: 'A static category/status label.' },
      { use: 'Freshness', when: 'A temporal/data-age signal — dot + relative time, optional pulse.' },
      { use: 'SaveIndicator', when: 'An inline, field-level "saved" tick (e.g. DataGrid cell edits).' },
      { use: 'Spinner / Skeleton', when: 'Indeterminate progress (Spinner) vs. layout-shaped placeholder that prevents shift (Skeleton).' },
    ],
  },
  {
    ...
    choices: [
      ...
      { use: 'PageHeader', when: 'A page title + subtitle + actions (with an optional Breadcrumbs slot).' },
    ],
  },
];
...
function ClusterTable({ cluster }: { cluster: Cluster }) {
  return (
    <section style={{ marginBottom: 'var(--eidra-space-6)' }}>
      <h3 style={{ font: '600 var(--eidra-font-size-md)/1.2 var(--eidra-font-family-sans)', color: 'var(--eidra-fg)', margin: '0 0 var(--eidra-space-1)' }}>
        {cluster.title}
      </h3>
      <p style={{ font: '400 var(--eidra-font-size-sm)/1.4 var(--eidra-font-family-sans)', color: 'var(--eidra-fg-muted)', margin: '0 0 var(--eidra-space-3)' }}>
        Shared space: {cluster.space}
      </p>
      <table style={{ width: '100%', maxWidth: 760, borderCollapse: 'collapse', fontFamily: 'var(--eidra-font-family-sans)', fontSize: 'var(--eidra-font-size-sm)' }}>
        <tbody>
          {cluster.choices.map((c) => (
            <tr key={c.use} style={{ borderTop: '1px solid var(--eidra-border)' }}>
              <td style={{ padding: 'var(--eidra-space-2) var(--eidra-space-3) var(--eidra-space-2) 0', whiteSpace: 'nowrap', verticalAlign: 'top', fontWeight: 'var(--eidra-font-weight-semibold)', color: 'var(--eidra-fg)', fontFamily: 'var(--eidra-font-family-mono)' }}>
                {c.use}
              </td>
              <td style={{ padding: 'var(--eidra-space-2) 0', verticalAlign: 'top', color: 'var(--eidra-fg)' }}>{c.when}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export const Guide: Story = {
  render: () => (
    <div style={{ maxWidth: 820 }}>
      <p style={{ font: '400 var(--eidra-font-size-base)/1.5 var(--eidra-font-family-sans)', color: 'var(--eidra-fg-muted)', margin: '0 0 var(--eidra-space-6)' }}>
